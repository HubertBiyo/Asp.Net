...
namespace Hubert.DefendService.Log
{
    public static class Logger
    {
        ...
        private static void WriteLog(string message, Type sender, Exception exception, LogType type)
        {
            ...
            _memoryLog.AppendLine(logMessage);
            RemoveOldLogs();
#if DEBUG
        ...
        {
            if (_memoryLog.Length <= MaxCapacity)
            {
                return;
            }

            _memoryLog.Remove(0, _memoryLog.Length - MaxCapacity);
        }
        ...
        private static void ConfigLog()
        {
            ...
            LoggingRule timelineRule = new LoggingRule("*", LogLevel.Warn, timelineTarget);
        ...
        static Logger()
        {
            _memoryLog = new StringBuilder(MaxCapacity);
        ...
        private static readonly StringBuilder _memoryLog;
        private const int MaxCapacity = 524288;//1Mb for Unicode Text
